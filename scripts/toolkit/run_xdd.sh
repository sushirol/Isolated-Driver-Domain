...
calc_files()
{
	...
	do
		TFILE=""
		TFILE="$TARGET_DIR/test"
		TFILE="$TFILE/$TFILE_PREFIX""_""$i"
		TFILE="$TFILE""_""$TS"
		TARGET_FILES="$TARGET_FILES ""$TFILE "
...
run_xdd()
{
	...
	then
		...
		echo "$XDD_EXE $OPERATION -targets $TARGET_COUNT $TARGET_FILES -blocksize $BS $FILE_SIZE_TYPE $FILE_SIZE -passes 1 $CREATE_NEW $VERBOSE $DATA_PATTERN $DIRECT_IO" | tee $CMD_LOGFILE
		...
		echo "$XDD_EXE $OPERATION -targets $TARGET_COUNT $TARGET_FILES -blocksize $BS $FILE_SIZE_TYPE $FILE_SIZE -passes 1 $CREATE_NEW $VERBOSE $DATA_PATTERN $DIRECT_IO" 
	fi
