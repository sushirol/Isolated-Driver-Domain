 ...
 * UNIX routines that are not in the Windows NT/2000/XP environment.
 */
#include "xdd.h"
/*-----------------------------------------------------------------*/
...
pthread_mutex_init(pthread_mutex_t *mp, int n) {
	...
	sprintf(mp->name,"xddMutex%I64x%",mp);
	...
	if (sp->sem_op == -1) { 
		...
		if (status == -1) {
			FormatMessage( 
				...
				0,
				NULL);
			fprintf(stderr,"semop: Wait failed for semaphore semid %d\n", semid);
			fprintf(stderr,"reason:%s",lpMsgBuf);
		}
	} else {
...
#ifdef notdef
/*-----------------------------------------------------------------*/
/* atoll() - This function maps to the Windows atoi64() function.
 */
int64_t
atoll(char *c) {
	return(_atoi64(c));
