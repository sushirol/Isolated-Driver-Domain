...
//	struct idd_request *req = container_of(taskp, struct idd_request, write_task);
	backend_info_t *be_info = container_of(taskp, backend_info_t, write_task);
	struct idd_request *req = be_info->rw_req;
...
//retry:
	if(req->data_direction == 1){
		schedule_work(&backend.write_task);
		printk("inserted in queue seq_no %lu!\n",req->seq_no);
	}else{
		if(count <10){
			count++;
			schedule_work(&backend.read_task);
//	notify_remote_via_irq(backend.ring_irq);
			printk("inserted in queue seq_no %lu!\n",req->seq_no);
		}else{
//			goto retry;
			printk("packed dropped seq_no %lu!\n",req->seq_no);
		}
	}
...
//        f = do_sys_open(AT_FDCWD, (const char*)"/dev/sdb", O_RDWR, 0);
	...
	INIT_WORK(&backend.read_task, process_read_request);
	INIT_WORK(&backend.write_task, process_write_request);
