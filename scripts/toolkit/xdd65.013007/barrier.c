...
xdd_init_barrier(struct xdd_barrier *bp, int32_t threads, char *barrier_name) {
	...
	bp->semid_base = (int32_t)barrier_name + getpid();
...
/*
	if (status == -1) {
			sprintf(errmsg,"Error initializing %s barrier semaphore: semctl",barrier_name);
			perror(errmsg);
		return(-1);
	}
*/
...
xdd_init_all_barriers(void) {
	...
	if (status) {
		sprintf(errmsg,"Error initializing xgp->xdd_init_barrier_mutex");
		perror(errmsg);
		return;
	}
	...
	status = xdd_init_barrier(&xgp->prefinal_barrier, xgp->number_of_iothreads, "PreFinal");
	if (status < 0) 
		xgp->global_options |= RX_NOBARRIER;
