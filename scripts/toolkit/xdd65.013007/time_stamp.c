...
xdd_ts_overhead(struct tthdr *ttp) { 
	...
	ttp->timer_oh /= 100;
	if (xgp->global_options & RX_TIMER_INFO) { /* only display this information if requested */
#ifdef WIN32 
		fprintf(xgp->errout,"Timer overhead is %I64d nanoseconds\n",
#else
...
xdd_ts_setup(ptds_t *p) {
	...
	if (!(p->ts_options & TS_ON) && !(xgp->global_options & RX_DESKEW))
		return;
	/* If DESKEW is TRUE but the TS option was not requested, then do a DESKEW ts setup */
	if ((xgp->global_options & RX_DESKEW) && !(p->ts_options & TS_ON)) {
	...
	if (xgp->global_options & RX_DESKEW) { /* This is a case where the target has time stamping already enabled as well as deskew */
...
#if (IRIX || SOLARIS || HPUX || AIX || ALTIX || OSX)
...
xdd_ts_write(ptds_t *p) {
	...
	if ((p->ts_options & TS_DUMP) == 0)  /* dump only if DUMP was specified */
		return;
	sprintf(tsfilename,"%s.target.%04d.qthread.%04d.bin",xgp->tsoutput_filename,p->mynum,p->myqnum);
...
xdd_ts_reports(ptds_t *p) {
	...
	if (p->thread_complete == 0) {
		...
		if (xgp->tsoutput_filename != 0) {
			sprintf(filename,"%s.target.%04d.qthread.%04d.csv",xgp->tsoutput_filename,p->mynum,p->myqnum);
...
#ifdef WIN32
				fprintf(p->tsfp, "Last time stamp table entry, %I64d\n",indx);
#else
				fprintf(p->tsfp, "Last time stamp table entry, %lld\n",indx);
#endif
			}
#ifdef WIN32
	fprintf(p->tsfp,"Start of DETAILED Time Stamp Report, Number of entries, %d, Picoseconds per clock tick, %I64d, delta, %I64d\n",ttp->numents,ttp->res,ttp->delta);
	fflush(p->tsfp);
#else
		...
		for (i = 0; i < ttp->numents; i++) {
			...
			if (i == 0) {
				...
				if (ttp->blocksize == 0) {
					fprintf(xgp->errout,"%s: ALERT! ts_reports encounterd a blocksize of zero for target %d thread %d, setting it to %d\n",
						xgp->progname, p->mynum, p->myqnum, p->block_size);
					fflush(xgp->errout);
			...
			if (p->ts_options & TS_DETAILED) { /* Print the detailed report */
				...
				switch (ttp->tte[i].rwvop) {
					case RX_WRITE: opc="w"; break;
					default: opc="r"; break;
				}
#ifdef WIN32
	fprintf(p->tsfp,"%d,%s,%d,%d,%I64d,%I64d,%I64u,%I64u,%15.5f,%15.5f,%15.5f,%15.5f,%12.3f\n",
#else
	fprintf(p->tsfp,"%d,%s,%d,%d,%lld,%lld,%llu,%llu,%15.5f,%15.5f,%15.5f,%15.5f,%12.3f\n",
		...
		if (p->ts_options & TS_SUMMARY) {  /* Generate just the summary report */
			...
			/* display the results */
#ifdef WIN32
			fprintf(p->tsfp,"Average seek distance in %d byte blocks, %I64d, request size in blocks, %d\n",
#else
			...
			fflush(p->tsfp);
#ifdef WIN32
			fprintf(p->tsfp,"Range:  Longest seek distance in blocks, %I64d, shortest seek distance in blocks, %I64d\n",
#else
