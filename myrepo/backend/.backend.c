...
int f;
static int count=0;
void process_write_request(struct work_struct *taskp){

	struct idd_response *rsp;
	int err=-1, notify;
        mm_segment_t old_fs;
        loff_t pos = 0;
	struct idd_request *req = container_of(taskp, struct idd_request, write_task);
...
void process_read_request(struct work_struct *taskp){

	struct idd_response *rsp;
	int err=-1, notify;
        mm_segment_t old_fs;
        loff_t pos = 0;
	struct idd_request *req = container_of(taskp, struct idd_request, read_task);
...
static irqreturn_t irq_ring_interrupt(int irq, void *dev_id)
{
...
#if 0
	if(req->data_direction == 1){
		INIT_WORK(&req->write_task, process_write_request);
		schedule_work(&req->write_task);
		printk("inserted in queue seq_no %lu!\n",req->seq_no);
	}else{
		INIT_WORK(&req->read_task, process_read_request);
		schedule_work(&req->read_task);
		printk("inserted in queue seq_no %lu!\n",req->seq_no);
	}
#endif
#if 1
	...
	rsp->op = req->data_direction;
#if 1
	printk("Debug: before push rsp_prod_pvt %d\n", backend.main_ring.rsp_prod_pvt);
	printk("Debug: before req_prod %d rsp_prod %d\n", backend.main_ring.sring->req_prod, backend.main_ring.sring->rsp_prod);
	smp_mb();
#endif
	RING_PUSH_RESPONSES_AND_CHECK_NOTIFY(&backend.main_ring, notify);
	printk("Debug: after push rsp_prod_pvt %d\n", backend.main_ring.rsp_prod_pvt);
	printk("Debug: after req_prod %d rsp_prod %d\n", backend.main_ring.sring->req_prod, backend.main_ring.sring->rsp_prod);
	notify_remote_via_irq(backend.ring_irq);
	printk("interrupt handled at backend %lu. sending interrupt to frontend %lu!\n",req->seq_no, rsp->seq_no);
#endif
        return IRQ_HANDLED;
